...
export default function Candidates() {
	...
	const handleCoverLetterChange = (event: ChangeEvent<HTMLInputElement>) => {
		...
		if (file.size > maxFileSize) {
			setSubmitError('La lettre de motivation dépasse la taille maximale de 5 Mo.')
			event.target.value = ''
			setCoverLetterFile(null)
			return
		}
	...
	const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
		...
		try {
			...
			const structuredAnswers = (job.prescreenQuestions ?? []).map((question, index) => ({
			...
			setPersonalInfo({ firstName: '', lastName: '', email: '', phone: '', city: '' })
			setAnswers({})
			setCvFile(null)
			setCoverLetterFile(null)
		...
		return (
			<main className="min-h-screen bg-slate-50 px-4 py-8 md:px-8">
				<div className="mx-auto max-w-3xl rounded-2xl bg-white p-8 text-center shadow-sm">
					<h1 className="text-xl font-semibold text-slate-800">Offre indisponible</h1>
					<p className="mt-2 text-slate-600">{jobError || 'Cette offre n’est plus accessible.'}</p>
				</div>
			</main>
		)
	}
		...
		<main className="min-h-screen bg-slate-50 px-4 py-8 md:px-8">
			<div className="mx-auto w-full max-w-4xl space-y-5">
				<section className="rounded-2xl bg-white p-5 shadow-sm md:p-6">
					<h1 className="text-2xl font-bold text-slate-800 md:text-3xl">{job.title}</h1>
					<p className="mt-2 text-sm text-slate-600 md:text-base">
						{job.department} • {job.location} • {(job.contractType ?? '-').toUpperCase()}
					</p>
					<div
				...
				>
					<section>
						<h2 className="mb-3 text-lg font-semibold text-slate-800">Informations personnelles</h2>
					...
					<section>
						<h2 className="mb-3 text-lg font-semibold text-slate-800">Pièces jointes</h2>
						<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
							<div>
								...
								<input
									...
									required
								/>
								{cvFile && <p className="mt-1 text-xs text-slate-500">{cvFile.name}</p>}
							</div>
							...
							<div>
								...
								/>
								{coverLetterFile && (
									<p className="mt-1 text-xs text-slate-500">{coverLetterFile.name}</p>
								)}
							</div>
						</div>
					</section>
						...
						<section>
							...
							<div className="space-y-4">
								{(job.prescreenQuestions ?? []).map((question, index) => {
									...
									return (
										<div key={`${question.label}-${index}`} className="rounded-xl border border-slate-200 p-4">
					...
					{submitMessage && (
						<p className="rounded-lg bg-emerald-50 p-3 text-sm text-emerald-700">{submitMessage}</p>
					)}
					{submitError && (
